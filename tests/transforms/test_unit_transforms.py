...
class TestCreateRandomMaskd:
    ...
    def test_output_keys(self, op, mae_sample_data):
        ...
        assert out.keys() == {"img", "mask", "brain_mask", "sub_id", "ses_id", "mod"}
    ...
    def test_mask_ratio(self, op, mae_sample_data, mask_ratio):
        ...
        assert (
            (1 - mask_ratio) - 0.1 < mask_ratio_actual < (1 - mask_ratio) + 0.1
    ...
    def test_patch_uniformity(self, op, mae_sample_data, mask_patch_size):
        """Check that every non-overlapping patch of size mask_patch_size^3 is
        entirely masked or entirely un-masked."""
        out = op(mae_sample_data)
        mask = out["mask"].float()
...
class TestSaveReconstructionTargetd:
    ...
    def test_output_keys(self, op, mae_sample_data):
        out = op(mae_sample_data)
        assert out.keys() == {"img", "recon", "brain_mask", "sub_id", "ses_id", "mod"}
...
class TestGetKeyQueryd:
    def test_output_keys_random(self, contrastive_sample_data):
        out = GetKeyQueryd(keys_prefix="img", count_key="count")(
            contrastive_sample_data
        )
        assert out.keys() == {"query", "key", "sub_id", "ses_id", "mod"}

    def test_output_keys_augmented(self, contrastive_sample_data):
        out = GetKeyQueryd(
            always_augment_query=True,
            query_key="img_0",
            extra_keys=["mod_0", "sub_id", "ses_id"],
        )(contrastive_sample_data)
        assert out.keys() == {"query", "key", "sub_id", "ses_id", "mod_0"}

    def test_output_values_random(self, contrastive_sample_data):
        out = GetKeyQueryd(keys_prefix="img", count_key="count", track=True)(
            contrastive_sample_data
        )
        assert (
            out["query"] == contrastive_sample_data[f"img_{out['track']['query_idx']}"]
        ).all()
        assert (
            out["key"] == contrastive_sample_data[f"img_{out['track']['key_idx']}"]
        ).all()
...
class TestSlidingWindowPatch:
    ...
    def test_different_inputs_same_overlap(self, tensor_shape):
        """Test that the output shape (different inputs, same overlap) is
        correct.
    ...
    def test_same_input_different_overlap(self, overlap):
        """Test that the output shape (same input, different overlap) is
        correct."""
        op = SlidingWindowPatch(
            patch_size=128,
            overlap=overlap,
    ...
    def test_different_input_same_n_patches(self, tensor_shape):
        """Test that the output shape (different input, same n_patches) is
        correct."""
        op = SlidingWindowPatch(
            patch_size=128,
            overlap=None,
    ...
    def test_same_input_different_n_patches(self, n_patches):
        """Test that the output shape (same input, different n_patches) is
        correct."""
        op = SlidingWindowPatch(
            patch_size=128,
            overlap=None,
    ...
    def test_error_no_patch_or_overlap(self):
        """Test that the error is raised when no patch or overlap is
        provided."""
        with pytest.raises(ValueError):
            SlidingWindowPatch(
                patch_size=128,
...
class TestRandImgKeyd:
    ...
    def test_error_no_keys_match_allow_missing(self, contrastive_sample_data):
        out = RandImgKeyd(keys=["img_40", "img_41"], allow_missing_keys=True)(
            contrastive_sample_data
        )
        assert out.get("img_0") is not None
        assert out.get("img_1") is not None
    ...
    def test_no_error_replace_key(self, contrastive_sample_data):
        out = RandImgKeyd(keys=["img_0", "img_1"], new_key="img_0")(
            contrastive_sample_data
        )
        assert out.get("img_0") is not None
        assert out.get("img_1") is not None
