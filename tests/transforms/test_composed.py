...
class TestContrastiveTrainTransforms:
    ...
    def test_output_values(
        self, ref_contrastive_train_transforms, contrastive_sample_data
    ):
...
class TestContrastiveValTransforms:
    ...
    def test_output_values(
        self, ref_contrastive_val_transforms, contrastive_sample_data
    ):
...
class TestClassificationTrainTransforms:
    ...
    def test_output_shapes_concat(self, contrastive_sample_data):
        """Tests if concat_img=True gives 4D tensor (n_mod, *patch_size) with
        patch_size=120."""
        op = Compose(
            get_classification_train_transforms(concat_img=True, patch_size=120)
        )
        out = cast(dict, op(contrastive_sample_data))
        assert out["img"].ndim == 4
        assert out["img"].shape == (3, 120, 120, 120)

    def test_output_shapes_sliding_window(self, contrastive_sample_data):
        """Tests if sliding_window=True gives 4D tensor (n_patches, C,
        *patch_size) with patch_size=120."""
        op = Compose(
            get_classification_train_transforms(sliding_window=True, patch_size=120)
        )
        out = cast(dict, op(contrastive_sample_data))
        assert out["img_0"].shape == (8, 1, 120, 120, 120)
        assert out["img_1"].shape == (8, 1, 120, 120, 120)
        assert out["img_2"].shape == (8, 1, 120, 120, 120)

    def test_output_shapes_sliding_window_concat(self, contrastive_sample_data):
        """Tests if sliding_window=True and concat_img=True gives 5D tensor
        (n_patches, n_mod, *patch_size) with n_patches=(2, 2, 2) and
        patch_size=120."""
        op = Compose(
            get_classification_train_transforms(
                sliding_window=True,
    ...
    def test_output_shapes_sliding_window_concat_n_patches(
        self, contrastive_sample_data
    ):
        """Tests if sliding_window=True and concat_img=True gives 5D tensor
        (n_patches, n_mod, *patch_size) with n_patches=(2, 3, 2) and
    ...
    def test_output_types_sliding_window_concat(self, contrastive_sample_data):
        """Tests if sliding_window=True and concat_img=True preserves
        MetaTensor."""
        op = Compose(
            get_classification_train_transforms(sliding_window=True, concat_img=True)
        )
        out = cast(dict, op(contrastive_sample_data))
        assert isinstance(out["img"], MetaTensor)

    @pytest.mark.parametrize(
        "patch_size_int,patch_size_tuple",
        [
            (32, (32, 32, 32)),
            (64, (64, 64, 64)),
            (144, (144, 144, 144)),
    ...
    def test_ensure_tuple_dim(
        self, contrastive_sample_data, patch_size_int, patch_size_tuple
    ):
        """Tests if ensure_tuple_dim() works."""
        op_int = Compose(
            get_classification_train_transforms(
                patch_size=patch_size_int, concat_img=True
            )
        )
        op_tuple = Compose(
            get_classification_train_transforms(
                patch_size=patch_size_tuple, concat_img=True
            )
        )
