...
class TestInfoNCELoss:
    ...
    def test_forward_top_k_negatives_exceeds_queue_size(
        self, query_tensor, key_tensor, queue_tensor
    ):
    ...
    def test_forward_w_reference(self, query_tensor, key_tensor, queue_tensor):
        ...
        ref_loss = F.cross_entropy(
            torch.cat([pos_logits, neg_logits], dim=1) / 0.07,
    ...
    def test_forward_w_reference_cl_stats(self, query_tensor, key_tensor, queue_tensor):
        ...
        contrastive_acc = (
            (torch.cat([pos_logits, neg_logits], dim=1).argmax(dim=1) == 0)
            .float()
            .mean()
        )
