...
Unit tests for several functions that can be visually inspected (e.g.,
logging, create directories) are omitted.

Tests are grouped based on functionality, which is not always the same
as the location in the source code. For example, utility functions
related to model training are grouped under tests/engines/test_utils.py
regardless if functions are defined in anyBrainer.engines.utils or
anyBrainer.utils.models.
...
mp.set_start_method(
    "fork", force=True
)  # Replace with worker_init_fn when seeding ready.
...
def ref_mae_val_transforms():
    return [
        LoadImaged(
            keys=["img", "brain_mask"], reader="NumpyReader", ensure_channel_first=True
        ),
        CreateEmptyMaskd(mask_key="brain_mask"),
        SpatialPadd(
            keys=["img", "brain_mask"], spatial_size=(128, 128, 128), mode="edge"
        ),
...
def ref_contrastive_train_transforms():
    return [
        ...
        SpatialPadd(keys=["query", "key"], spatial_size=(128, 128, 128), mode="edge"),
...
def ref_predict_transforms():
    return [
        ...
        SpatialPadd(keys=["img"], spatial_size=(128, 128, 128), mode="edge"),
    ]
