...
def parallel_process(process_func, tasks, num_workers=None, skipped=0, desc="Processing"):
    """
    ...
    if skipped != 0: 
        print(f"{skipped}/{len(tasks) + skipped} items are already "
               "preprocessed and will be skipped.")
    
    print(f"Processing {len(tasks)} items using {num_workers} workers")
    
    results = []
    with multiprocessing.Pool(processes=num_workers) as pool:
        for i, r in enumerate(pool.imap_unordered(process_func, tasks), 1):
            if i % 1000 == 0:
                print(f"[{i}/{len(tasks)}] scans done")
            results.append(r)
