...
def modality_to_idx(
    ...
    if not isinstance(modalities, list):
        raise TypeError(
            f"Expected list of modalities under key '{key}', got {type(modalities)}"
        )
...
def modality_to_onehot(
    ...
    if not isinstance(modalities, list):
        msg = (
            f"Expected a list of modalities under key '{key}', "
    ...
    one_hots = torch.zeros(
        (len(modalities), len(MODALITY_LABELS)), dtype=torch.float32, device=device
    )
...
def split_data_by_subjects(
    ...
    logger.info(
        f"Splitting data into train/val/test based on subjects "
        f"with current state: {pos:3d}, {mt_state[:5]}"
    )  # type: ignore
...
def apply_tta_monai_batch(
    ...
    """Apply MONAI dictionary transforms (TTA) to a batched dict and return
    both the augmented batch and an inversion function for model outputs.
    ...
    for tr in tta_transforms:
        if isinstance(tr, Compose):
            if not all(isinstance(t, MapTransform) for t in tr.transforms):
                msg = (
                    "Compose contains non-MapTransform; use *d transforms "
                    "for dict pipelines."
                )
                logger.error(msg)
                raise TypeError(msg)
            ...
            msg = (
                f"TTA transform {tr.__class__.__name__} is not a MapTransform; "
    ...
    composed_tta = Compose(
        [EnsureTyped(keys=img_key, track_meta=True)] + tta_transforms
    )
    if seed is not None:
    ...
    def invert_fn(
        ...
        """Invert TTA on `out_key` using the transform trace recorded on
        `img_key`."""
        inv_op = Invertd(
            keys=out_key,
