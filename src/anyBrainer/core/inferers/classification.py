...
__all__ = ["SlidingWindowClassificationInferer"]
...
class SlidingWindowClassificationInferer(Inferer):
    """Collects classification predictions from a sliding window and optionally
    assigns a single label.
    ...
    def __init__(
        ...
        aggregation_mode: Literal[
            "weighted", "mean", "majority", "none", "noisy_or", "lse", "topk"
        ] = "noisy_or",
        ...
        *,
        ...
        logger.info(
            f"[{self.__class__.__name__}] Initialised with "
    ...
    def __call__(
        ...
        if inputs.ndim != self.spatial_dims + 2:  # (B, C, *spatial_dims)
            msg = (
                f"Inputs must have {self.spatial_dims + 2} dimensions, "
                f"got {inputs.ndim}"
            )
            logger.error(msg)
            raise ValueError(msg)
        ...
        if self.aggregation_mode == "topk":
            ...
            for slc in slices:
                ...
                center = [
                    start + ps / 2.0
                    for start, ps in zip(start_idxs, self.slice_extractor.patch_size)
                ]
