...
def check_flat_npy_data_dir(data_dir: Path | str) -> None:
    ...
    Assumes that the data is in a flat directory structure, with .npy
    files saved as sub_x_ses_y_ModalityName_CountIfMoreThanOne.npy.
    """
    ...
    logger.info(
        f"Dataset contains {len(subjects)} subjects, "
...
def read_label_from_txt(
    ...
    if expected_labels is not None and content not in expected_labels:
        msg = (
            f"Unexpected label '{content}' in {file_path}. "
