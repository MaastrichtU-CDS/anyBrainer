...
class DefaultLoggingManager(LoggingManager):
    ...
    def __init__(self, **settings):
        ...
        self.main_logger.info(
            f"[{self.__class__.__name__}] Logging manager "
    ...
    def setup_main_logging(self) -> logging.Logger:
        ...
        format_tail = (
            "%(processName)s | %(message)s" if self.settings.dev_mode else "%(message)s"
        )
    ...
    def setup_parallel_logging(self) -> tuple[Queue, QueueListener]:
        ...
        stream_fmt = "%(asctime)s | %(levelname)s | %(processName)s | " "%(message)s"

        for handler in handlers:
            handler.setFormatter(logging.Formatter(stream_fmt, "%Y-%m-%d %H:%M:%S"))
